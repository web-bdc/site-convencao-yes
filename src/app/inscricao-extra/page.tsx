...
export default function InscricaoExtraPage() {

    const [dados, setDados] = useState<Partial<Inscricao>>({
        acomodacao: "Dupla", // Fixo para extra
        participantes: [],
        forma_pagamento: 3,
        parcelas: 10,
        installments: 10
    ...
    const zodResult = useMemo((): {
        ...
        const guestLabels: Record<string, string> = {
            nome_participante: "Nome do participante extra",
        ...
        for (const it of issues) {
            ...
            if (path[0] === "participantes" && typeof path[1] === "number") {
                ...
                messages.push(`• Participante Extra ${idx + 1} - ${label}: ${msg}`);
            } else {
    ...
    // Sempre true para extra
    const showConvidados = true;
    ...
    const doSubmit = async () => {
        ...
        try {
            ...
            if (!dados.nome_responsavel || !dados.cpf_responsavel || !unidadeIdNum || !dataResp) {
                setMsg("Preencha todos os dados do Responsável (Pagante).");
                setLoading(false);
                return;
            }
            ...
            const payload = {
                ...
                participantes: (dados.participantes || []).map((p, i) => ({
                    ...
                    cargo_participante: p.cargo_participante || "Participante",
                    tamanho_camisa_participante: p.tamanho_camisa_participante || "M",
                })),
            } as Inscricao;
    ...
    const onSubmit = async () => {
        ...
        setShowErrors(true);
        if (!isValid) {
            setMsg('Existem campos obrigatórios pendentes. Verifique os dados do responsável e dos participantes.');
            return;
        }

        // Validação específica para Extras: deve ter pelo menos 1 participante extra?
        // Se a página é "apenas para convidados", faz sentido exigir.
        if (!dados.participantes || dados.participantes.length === 0) {
            setMsg('Adicione pelo menos um Participante Extra.');
            return;
        }
        ...
        <>
            ...
            <main className={style.mainContainer}>
                <section className={style.sectionMain}>
                    ...
                    <p>
                        Preencha os dados do responsável financeiro e adicione os participantes extras abaixo.
                    </p>

                    <section className={style.formSection}>
                        <h3 className={style.formTitle}>Dados do Responsável Financeiro</h3>
                        <FormResponsavel
                            value={dados}
                            onChange={handleChangeResponsavel}
                            errors={showErrors && zodResult.ok === false ? zodResult.errorsTop : ({} as Partial<Record<keyof Inscricao, string>>)}
                        />
                    </section>
                    ...
                    <section className={style.formSection}>
                        <Subtotal dados={dados as unknown as Record<string, unknown>} ignoreResponsible={true} />
                    </section>
